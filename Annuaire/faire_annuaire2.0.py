...
# Julien Minet, Octobre 2012 - Novembre 2013, julien_wa@yahoo.fr
# Version 2.0: Adaptation au nouveau format de fichier xls en une seule feuille
...
# Pré-étape: classer le fichier selon les localités puis les provinces
...
header = '<html lang="fr">\n  <head>\n    <meta content="text/html; charset=utf-8" http-equiv="content-type">\n    <link rel="stylesheet" href="style-prive2.css" type="text/css">\n    <title>Annuaire des alternatives - ASBL RCR</title>\n  </head>\n  <body>\n'
f2.write(header)

# Ouvrir un fichier xls avec plusieurs feuilles
wb = open_workbook('RCR_carto_20150617.xls')
for s in wb.sheets():
    print 'Sheet:',s.name
    values = []
    ...
    for row in range(1,700):
        ...
        alternative = s.cell(row,4)   # Nom de la localite
        alternative = alternative.value.encode('utf-8')
        
        nom = s.cell(row,5)  # Nom de l'alternative
        nom = nom.value.encode('utf-8')
     
        texte = s.cell(row,6) # Petit texte du popup
        texte = texte.value.encode('utf-8')

        web = s.cell(row,8)   # Adresse site web
        ...
        email = s.cell(row,9)   # Email
        email = email.value.encode('utf-8')
        
        personne = s.cell(row,10)   # Personne de contact
        personne = personne.value.encode('utf-8')
        
        emailprive = s.cell(row,11)   # Email prive
        emailprive = emailprive.value.encode('utf-8')

        adresse = s.cell(row,12)   # Adresse
        adresse = adresse.value.encode('utf-8')
         
        telephone = s.cell(row,14)   # Telephone
        telephone = telephone.value.encode('utf-8')

        gsm = s.cell(row,15)   # GSM
        ...
        f1.write(html1+nom+' ('+alternative+')'+html2+localite+' ('+commune+')'+html3+texte+html4+web+html5)
        ...
        f2.write(html1+nom+' ('+alternative+')'+html2+localite+' ('+commune+')'+html3+texte+html4+personne+html5+emailprive+html6+adresse+html6+telephone+html7+web+'   '+email+html8+gsm+html9)
